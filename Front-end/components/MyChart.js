...
function MyChart({ onEdit, edit, setEdit }) {
  ...
  useEffect(() => {
    if (me) {
      ...
      const cur_ins = Math.floor(
        Math.abs(Math.abs(curDate / (1000 * 60 * 60 * 24)))
      );
  ...
  return (
    <form style={{ marginTop: '13px' }}>
      <StyledDiv>
        <div
          style={{ color: '#1B3B1A', alignItems: 'center', fontSize: '25px' }}
        >
          나의 군생활
        </div>
        <div style={{ color: '#777' }}>
          남은 복무일 : {total - current} / 현재 복무일 : {current}{' '}
        </div>
        <Chart
          options={options}
          series={series}
        ...
        />
        {edit === false && (
          <Button onClick={() => setEdit(!edit)}>입대일/전역일 수정</Button>
        )}
        {edit && (
          <>
